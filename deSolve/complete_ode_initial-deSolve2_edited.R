# slphyx@SHIFT-ENTER
# the modified version of complete_ode_initial for using with deSolve
# and foi from the Rcpp version
# parms  must be from read_data 
complete_ode_initial.deSolve2 <- function(month, Y, parms) {
  ...
  for (j in seq_along(parms$age_grps)) {
    ...
    # MV0I0: 1
    dY[j] <- 
      (ifelse(j > 1, parms$kappa[j-1]*Y[j-1], parms$monthlyBirths) - ifelse(j <parms$Nage ,parms$kappa[j]*Y[j],0) -  # ageing
    ...
    dY[parms$Nage + j] <-
      (ifelse(j > 1, parms$kappa[j-1]*Y[parms$Nage + j-1], 0) - ifelse(j <parms$Nage ,parms$kappa[j]*Y[parms$Nage + j],0) +
              ...
              Y[parms$Nage + j]*foi3(Y, month, j, parms))    # dead, infected, vaccinated
    ...
    dY[5*parms$Nage + j] <-
      (ifelse(j > 1, parms$kappa[j-1]*Y[5*parms$Nage + j-1], 0) - ifelse(j <parms$Nage ,parms$kappa[j]*Y[5*parms$Nage + j],0) +
         ...
         Y[5*parms$Nage + j]*parms$sigmaR*foi3(Y, month, j, parms) -
         Y[5*parms$Nage + j]*parms$mortality[j])      # risk adjusted infection, parms$mortality, vaccination
    ...
    # MV1I0: 10
    dY[9*parms$Nage + j] <-
      (ifelse(j > 1, parms$kappa[j-1]*Y[9*parms$Nage + j-1], 0) - ifelse(j <parms$Nage ,parms$kappa[j]*Y[9*parms$Nage + j],0) -  # maternal vaccination
         Y[9*parms$Nage + j]*(parms$mortality[j] + parms$omegaM[j]))                                                # dead, waning immunity
    
    # SV1I0: 11
    dY[10*parms$Nage + j] <-
      (ifelse(j > 1, parms$kappa[j-1]*Y[10*parms$Nage + j-1], 0) - ifelse(j <parms$Nage ,parms$kappa[j]*Y[10*parms$Nage + j],0) +                                                                                                         # vaccinated
    ...
    # SV1IR: 15
    dY[14*parms$Nage + j] <-
      (ifelse(j > 1, parms$kappa[j-1]*Y[14*parms$Nage + j-1], 0) - ifelse(j <parms$Nage ,parms$kappa[j]*Y[14*parms$Nage + j],0) +                                                                                                               # vaccinated
  ...
  # cat('\n loc_incidence ', loc_incidence[1:2])
  ...
  if(month >= 1){
    ...
    if(trunc(month) > gb.indx){
      # cat('\n init month : ',month,' gb.indx ',gb.indx ,' incidence_monthly ', incidence[1:2])
  ...
  # return as list for using with deSolve
  list(c(dY))
} 
